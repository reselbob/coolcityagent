...
export class HourlyTravelMonitor {
    ...
    private async getTopCitiesWithWeather(region: Region = 'Global') {
        try {
...
Return your response as a JSON array of strings, with each string being a suggested activity.`;
    ...
    private async checkAndRecommend(region: Region = 'Global') {
        console.log(`\nChecking travel recommendations for ${region} at ${new Date().toLocaleString()}`);
        ...
        try {
            ...
            if (this.lastRecommendation !== recommendation.bestCity) {
                console.log(`\n=== New Travel Recommendation for ${region} ===`);
                ...
                console.log(`Recommendation unchanged: ${recommendation.bestCity} is still the best choice for ${region}`);
            }
        } catch (error) {
    ...
    public async start(region: Region = 'Global') {
        console.log(`Starting hourly travel monitoring for ${region}...`);

        // Run immediately on start
        await this.checkAndRecommend(region);
